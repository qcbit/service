...
import (
	...
	"github.com/qcbit/service/business/data/dbtest"
	"github.com/qcbit/service/business/data/order"
	"github.com/qcbit/service/foundation/docker"
)
...
func crud(t *testing.T) {
	...
	if usrs[0].DateCreated.UnixMilli() != saved.DateCreated.UnixMilli() {
		...
		t.Errorf("Should get back the same date created: %s", err)
	}

	if usrs[0].DateUpdated.UnixMilli() != saved.DateCreated.UnixMilli() {
		t.Logf("got: %v", saved.DateCreated)
		t.Logf("exp: %v", usrs[0].DateCreated)
		t.Logf("dif: %v", saved.DateCreated.Sub(usrs[0].DateCreated))
		t.Errorf("Should get back the same date updated: %s", err)
	}
	...
	if diff := cmp.Diff(usrs[0], saved); diff != "" {
		t.Fatalf("Should get back the same user:\n%s", diff)
	}
	...
	if !errors.Is(err, user.ErrNotFound) {
		t.Fatalf("Should NOT be able to retrieve user by ID: %s", err)
	}
}
...
func paging(t *testing.T) {
	...
	if users3[0].ID == users3[1].ID {
		...
		t.Errorf("Should have different users for page 1")
	}
}
